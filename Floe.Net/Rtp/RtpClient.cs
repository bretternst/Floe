...
namespace Floe.Net
{
	...
	public abstract class RtpClient : IDisposable
	{
		...
		/// <summary>
		/// Send a packet to all peers asynchronously. If there is a problem with the send, the OnError method is called.
		/// </summary>
		...
		/// <param name="payload">The packet's payload.</param>
		protected abstract void OnReceived(short payloadType, int seqNumber, int timeStamp, byte[] payload);
		...
		private void ReadPacket(IPEndPoint peer, byte[] buffer)
		{
			...
			this.OnReceived(payloadType, newSeq, timeStamp, _payload);
		}
